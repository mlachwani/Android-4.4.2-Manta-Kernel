...
static ssize_t chg_status_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t chg_status_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count);

static DEVICE_ATTR(charge_status, 0644, chg_status_show, chg_status_store);

struct smb347_charger *sysfs_smb;
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	if (sysfs_smb->charging_enabled == 1) {
		b_chg_ok = true;
		r = snprintf(buf, 3, "%d\n", b_chg_ok);
				...
				const char *buf, size_t count)
{
	...
	if (strncmp(buf, "0", 1) == 0) {
		...
		ret = smb347_charging_disable(sysfs_smb);
	} else if (strncmp(buf, "1", 1) == 0) {
		/* start charging */
		pr_info("[Diag] start charging start\n");
		ret = smb347_charging_enable(sysfs_smb);
	}
